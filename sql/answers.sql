...
SELECT m.name AS model_name, b.name AS brand_name, b.headquarters FROM models AS m JOIN brands AS b ON (m.brand_id = b.brand_id) WHERE b.name = 'Ford' AND m.name = 'Mustang';
...
SELECT * FROM brands WHERE founded <= '1902' ORDER BY founded;
...
SELECT b.name, m.year, m.name FROM brands AS b JOIN models AS m ON (b.brand_id = m.brand_id) ORDER BY b.name;
...
SELECT b.name, m.name, m.year, b.discontinued, (b.discontinued - m.year) AS years_until_brand_discontinued FROM models AS m LEFT JOIN brands as b ON (m.brand_id = b.brand_id) WHERE b.discontinued IS NOT NULL ORDER BY b.name, m.name, m.year; 
...
SELECT b.name, b.founded FROM brands AS b LEFT JOIN models AS m ON (b.brand_id = m.brand_id) GROUP BY b.brand_id HAVING COUNT(m.brand_id) = 0;
...
SELECT b.brand_id FROM brands AS b JOIN models AS m ON (b.brand_id = m.brand_id) GROUP BY b.brand_id HAVING COUNT(m.model_id) > 5;
